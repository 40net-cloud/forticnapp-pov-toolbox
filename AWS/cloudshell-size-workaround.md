...
|---------|----------------------|---------------------|
| 🧩 **1. Understand the limitation** | `lacework/aws-xxxx/` has only ~1 GB persistent storage | AWS CloudShell keeps this directory between sessions, but it is very limited. Terraform provider caches (`.terraform/`) can quickly consume all space. |
| 🧩 **1. Understand the limitation** | `` has only ~1 GB persistent storage | AWS CloudShell keeps this directory between sessions, but it is very limited. Terraform provider caches (`.terraform/`) can quickly consume all space. |
